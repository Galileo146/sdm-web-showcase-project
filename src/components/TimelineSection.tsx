...
const TimelineItem = ({
  ...
  return <div className={`flex flex-col ${isLeft ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-8`}>
      <div className="md:w-1/2">
        ...
        loop: true
            ...
            <CarouselContent>
              {carouselImages.map((img, idx) => <CarouselItem key={idx}>
          ...
          </Carousel> : <div className="overflow-hidden rounded-[20px]">
            <img src={image} alt={title} className="w-full h-auto max-h-[400px] object-cover" />
          </div>}
      </div>
  ...
  return <section className="py-16">
      <div className="container-wide">
        ...
        align: "center"
          ...
          <CarouselContent>
            {timelineItems.map((item, index) => <CarouselItem key={index} className="md:basis-full">
                <TimelineItem year={item.year} title={item.title} description={item.description} image={item.image} hasCarousel={item.hasCarousel} carouselImages={item.carouselImages} isLeft={index % 2 === 0} // Alterna il layout
            />
              </CarouselItem>)}
          </CarouselContent>
